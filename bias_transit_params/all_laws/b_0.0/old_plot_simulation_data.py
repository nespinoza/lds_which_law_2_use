...
sbar = c.insert(graph.graphxy(height=pheight,width=pwidth/4., xpos = g_a_sqrt.xpos + delta_x, \
                                                              ypos = g_a_sqrt.ypos+g_a_sqrt.height-2*delta_y,\
       key=graph.key.key(pos=legend_pos,textattrs=[text.size(legend_text_size)]),\
       x = graph.axis.linear(min=-2,max=-1,title = None,manualticks=mymanualticks),\
...
if fit_option == "float":
    draw_text(g_p_log, 7500, ytext, '$b='+str(b)+'$',text_size=0)
