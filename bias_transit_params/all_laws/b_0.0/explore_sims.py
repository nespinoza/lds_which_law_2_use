...
plot(input_teff_log,((p_log-p_float_log)/p_log)*100,'.')
show()
import sys
sys.exit()
from pyx import *
...
g_a_quad = c.insert(graph.graphxy(height=pheight,width=pwidth,\
...
cbar = c.insert(graph.graphxy(height=pheight*1.5,width=pwidth/4., xpos = g_a_sqrt.xpos + delta_x, \
                                                                  ypos = g_a_sqrt.ypos+g_a_sqrt.height+2.*delta_y,\
       key=graph.key.key(pos=legend_pos,textattrs=[text.size(legend_text_size)]),\
...
sbar = c.insert(graph.graphxy(height=pheight,width=pwidth/4., xpos = g_a_tpl.xpos + delta_x, \
...
if fit_option == "float":
    ...
    draw_text(g_p_quad, min_x + (max_x-min_x)/2., max_y_p + .1, 'Quadratic law (free LDs fit)',text_size=0)
    draw_text(g_p_log, min_x + (max_x-min_x)/2., max_y_p + .1, 'Logarithmic law (free LDs fit)',text_size=0)
    draw_text(g_p_sqrt, min_x + (max_x-min_x)/2., max_y_p + .1, 'Square-root law (free LDs fit)',text_size=0)
    draw_text(g_p_tpl, min_x + (max_x-min_x)/2., max_y_p + .1, 'Three-parameter law (free LDs fit)',text_size=0)
else:
    draw_text(sbar, -1.5, np.max(value_range)+1, '$b='+str(b)+'$',text_size=0)
    draw_text(g_p_quad, min_x + (max_x-min_x)/2., max_y_p + .1, 'Quadratic law (fixed LDs fit)', text_size=0)
    draw_text(g_p_log, min_x + (max_x-min_x)/2., max_y_p + .1, 'Logarithmic law (fixed LDs fit)',text_size=0)
    draw_text(g_p_sqrt, min_x + (max_x-min_x)/2., max_y_p + .1, 'Square-root law (fixed LDs fit)',text_size=0)
    draw_text(g_p_tpl, min_x + (max_x-min_x)/2., max_y_p + .1, 'Three-parameter law (fixed LDs fit)',text_size=0)
...
for method in ['log','sqrt','quad','tpl']:
        ...
        for i in range(len(p)):
           if p[i] in p_show and a[i]==3.27:#>=p_min and p[i]<=p_max:
