...
\end{eqnarray*}
From the definition of the MSE, it can be seen that it is a measure of the distance from the estimator to the parameters' ``true" value. As such, this provides an 
...
provides, on average, the one that is closer to the underlying ``true" value on a mean-square sense. Furthermore, it is easy to shown that the MSE can be written 
as
\begin{eqnarray*}
\textnormal{MSE} &=& \mathbb{E}[(\theta - \hat{\theta})^2], \\
...
shown and studied the biases that each limb-darkening law (i.e., different estimators) imply on the retrieved transit parameters through simulation. We now estimate 
the variances implied by those limb-darkening laws.
...
MSE as a function of the noise in the simulated lightcurves. 

In our results, it can be seen that the optimal law in terms of the MSE at each noise level can be selected easily. For example, in our simulation, for $N=100$ in-transit points, 
...
At this last noise level, it can be seen that the square-root law crosses the line of the logarithmic law, and this law attains the lowest MSE until $\sim 200$ ppm, where the line 
...
laws occur. For example, the transition between the quadratic and three-parameter laws occurs with lower precision lightcurves for the case of $1000$ in-transit points 
(at $\sim 75$ ppm).

From our case study, it can be seen that the selection of the optimal limb-darkening law in a MSE sense can be estimated by simple simulation; however, we note that different 
transit geometries will give rise to different optimal limb-darkening laws at a given lightcurve precision and, thus, these simulations must be performed on a case-by-case basis. 
We provide code at Github with which this can be easily done for a given set of transit and stellar parameters, along with a given lightcurve noise level and number of in-transit points. 
