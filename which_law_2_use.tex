...
logarithmic, square-root and three-parameter do a better job at retrieving the mentioned parameters from transit lightcurves both in terms of 
bias and precision for a wide range of situations than the widely used quadratic and linear laws and therefore we recommend to study which one to 
use on a case-by-case basis. We also provide code to guide the decision of when to use each of these laws and select the optimal one in a 
...
This has allowed the study of precise mass-radius diagrams \citep[see, e.g.,][]{wm2014,dressing2015,WL2015b}, and in turn  allow the possibility of not only obtaining a precise determination of the internal composition of small, rocky exoplanets based on mass-radius
...
In this sense, one would ideally want a low-bias and high-precision (i.e. low variance) estimator, which is both close to the real value of the parameter on average and, 
...
\end{eqnarray*}
From the definition of the MSE, it can be seen that it is a measure of the square of the distance from the estimator to the parameters' ``true" value. As such, this provides an 
excellent metric among a given suite of estimators for a given parameter: for a given set of estimators, the one that gives the minimum mean-squared error 
...
shown and studied the biases that each limb-darkening law (i.e., different estimators) imply on the retrieved transit parameters through simulation. In this sense, we have 
...
Given that the study of the mean-squared error is complex from a mathematical point of view in this case, we study it through simulations similar to the ones done in the past sub-section, 
but this time also adding different amounts of noise to the lightcurves in order to estimate the variances of the estimators. In this sense, we take a Monte Carlo approach in order to study 
...
lightcurves with noise levels ranging from $10$ ppm to $3000$ ppm with different amount of in-transit points; here, however, we show our results for $N=100$ and $N=1000$ in-transit 
points for clarity, which span a range of possible scenarios for both space based (e.g., \textit{Kepler}) and ground-based observations. For each case, $300$ lightcurves are generated, 
...
MSE as a function of the noise in the simulated lightcurves for each limb-darkening law used to retrieve the parameter. 

In our results, it is evident how the MSE evolves from being variance-dominated for low-precision lightcurves (rightmost part of the plot) to bias-dominated for high-precision 
...
$\sim100$ ppm. From here and for more precise lightcurves, the MSE of this law attains the constant value $\sim 10^{-6}$, which corresponds to the bias floor of 
$10^{-3}$, which for $p=0.1$ corresponds to the $1\%$ bias observed on our simulations on the past sub-section (Figure \ref{small_b_sims}). 
Note that from this plot, the optimal law in terms of the MSE at each noise level can be selected easily. For example, in our simulation, for $N=100$ in-transit points, 
the logarithmic law is the best for the retrieval of the planet-to-star radius ratio in a MSE sense from lightcurves with noise levels of $\sim3000$ ppm up to $\sim 1000$ ppm. 
At this last noise level, it can be seen that the square-root law crosses the line of the logarithmic law, and this law attains the lowest MSE until $\sim 200$ ppm, where the 
quadratic law starts to get the lowest MSE. The quadratic law is thus the best law to use from $\sim 200$ ppm until lightcurves with precisions of $\sim25$ ppm, precision at 
which the three-parameter law takes over and attains the lowest MSE. Note that the number of in-transit points changes where the transition among different limb-darkening 
laws occur due to the fact that the bias floor is attained faster due to the increased number of points. For example, the transition between the quadratic and three-parameter 
laws occurs with lower precision lightcurves for the case of $1000$ in-transit points (at $\sim 75$ ppm).

From our case study, it can be seen that the selection of the optimal limb-darkening law in a MSE sense can be estimated by simple simulation: simply calculate the MSE obtained 
...
inputs, which might be strange as this is what we are trying to extract from a transit lightcurve. However, as we learned from our results on this and the past sub-sections, it is important 
to remember that the functional relation of the transit parameters and the biases/variances is smooth and slow; (i.e., small changes on the retrieved transit parameters give rise to 
similar biases and variances). As such, one might first one to fit with one favourites' law, obtain a set of transit parameters and use these as inputs in order to calculate the MSE of 
different limb-darkening laws in order to retrieve the final transit parameters from the law that gives the lowest MSE. We provide code at Github with which this can be easily done 
