...
shown that it is now possible to fit the limb-darkening coefficients in the transit-fitting procedure rather than fix them to theoretical values. In 
addition, this strategy has been shown to give better results as fixing the coefficients to theoretical values can give rise to important yet unknown systematic 
...
have mostly focused on the widely used quadratic limb-darkening law, leaving out other proposed laws that are in fact better descriptions 
of model intensity profiles, which in principle could diminish the bias even further. In this work, we show that laws such as the logarithmic, 
square-root and three-parameter do a better job in general at retrieving the mentioned parameters from transit lightcurves than the widely 
used quadratic law and therefore we recommend using those instead. We also provide code to guide the decision of when to use each of 
the laws,  which we note has a dependence on both stellar and transit parameters. Finally, we demonstrate that the so-called exponential 
law is non-physical as it typically produces negative intensities close to the limb and should therefore not be used.
\end{abstract}
...
in GitHub\footnote{\url{http://www.github.com/nespinoza/ld-exosim/}}, and can be used to perform analyses in a finer grid than the ones 
published here. 
