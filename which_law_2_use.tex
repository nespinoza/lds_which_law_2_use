...
\caption{Sketch of each of the terms that define the bias and the precision of a given estimator. The probability density of the estimator $\hat{theta}$ of 
...
$\mathbb{E}[\cdot]$ defines the expectation value and $\textnormal{Var}[\cdot]$ is the variance operator. Intuitively, one would want a low-bias and 
high-precision (i.e., low variance) estimator for a given parameter; Figure \ref{bias-variance-plot} sketches each of those terms graphically. In this subsection, 
we will be interested in the ratio of the \textit{absolute value} of the bias over the precision, which we define as
\begin{eqnarray*}
...
but this time also adding different amounts of noise to the lightcurves. In this sense, we take a Monte Carlo approach in order to study this ratio. As a case study, we take that 
of a typical Hot-Jupiter ($p=0.1$, $a_R = 10$) around a Solar-type star ($T_\textnormal{eff}= 5500$ K) on a low impact-parameter orbit ($b=0.3$), and we focus specifically 
on the retrieval of the planet-to-star radius ratio, which is one of the most interesting parameters retrieved from transit lightcurves. We simulate a suite of lightcurves with noise 
levels ranging from $10$ ppm to $3000$ ppm with different amount of in-transit points; here, however, we show our results for $N=100$ and $N=1000$ in-transit points for clarity, 
which span a range of possible scenarios for both space based (e.g., \textit{Kepler}) and ground-based observations. For each case, $300$ lightcurves are generated, 
...
we take all the estimated planet-to-star radius ratios and estimate the precision by calculating the square-root of the variance of the sample. The results of our simulations are shown 
on Figure \ref{biasvariance}, where we plot the ratio of the absolute value of the bias (estimated in the previous sub-section) over the precision 
(the square-root of the variance of the estimator) as a function of the noise in the simulated lightcurves. 

In our results, the line of equal bias and precision marks the limit at which one should switch to a more complex law. For example, in our simulation, for $N=100$ in-transit points, the 
linear law touches this line at $\sim 500$ ppm, which means that for more precise lightcurves the bias on the planet-to-star radius ratio is larger than the precision (e.g., for a lightcurve with a precision of $\sim 200$ 
...
it can be seen that between $\sim 50-500$ ppm, the best law in terms of bias/variance trade-off for this number of in-transit points is the square-root law which, among all the laws that are below 
the equal bias and precision line, provides the best precision. For lightcurve precisions better than $\sim 50$ ppm, it can be seen that the square-root law crosses this line and therefore another law 
must be used, which can be selected in the same way as before (in this case, between $\sim 30-50$ ppm, the optimal decision would be to use the logarithmic law). For this specific case, it can be seen that the 
three-parameter law is only optimal for precision better than $\sim 20$ ppm. Of course, as can be seen from the plot, if one increases the number of in-transit points, the precisions at which each law crosses the 
equal bias and precision line are lower. For example, for $N=1000$, it can be seen that the linear law touches this line at $\sim 1500$ ppm, meaning that in this case even with a very noisy lightcurve one has to 
switch to a better limb-darkening law if one wants to keep the bias below the precision level.
