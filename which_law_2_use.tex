...
This has allowed the study of precise mass-radius diagrams \citep[see, e.g.,][]{wm2014,dressing2015,WL2015b}, and in turn  allow the possibility of  obtaining a precise determination of the internal composition of small, rocky exoplanets based on mass-radius
...
want a low bias, high precision estimator.{\bf TBD Here under sqrt(variance) it should say 1/precision}}
...
square-root defines the precision), $\textnormal{Var}[\hat{\theta}]$, and its bias, defined as $\mathbb{E}[\hat{\theta}]-\theta$, where 
...
MSE as a function of the noise of the simulated lightcurves for each of the limb-darkening law used to retrieve the parameter. 
...
Note that from this plot, the optimal law in terms of the MSE at each noise level can be selected with easse. For example, in our simulation, for $N=100$ in-transit points, 
...
to remember that the functional relation of the transit parameters and the biases/variances is smooth and slow. Therefore, one can first fit with one favourites' law, obtain a set of transit parameters, and use those as inputs in order to calculate the MSE of 
different limb-darkening laws in order to then retrieve the final transit parameters from the law that gives the lowest MSE. We provide code at Github with which this process be easily done 
for a given set of transit and stellar parameters, along with a given lightcurve noise level and number of in-transit points. 
