...
rotation = [-15.,-13.,-15.,-12.5,-10.]
delta_y = [7e-2,5e-3,2.5e-2,1e-2,3e-3]
...
yaxis = r'Bias/Precision'
outname = 'bias_variance_tradeoff'
...
for i in range(len(ld_laws)):
    ...
    text_pyx(g, precisions[-2], np.abs(data[ld_law]['100']['bias_p']/data[ld_law]['100']['precision_p'])[-2]+delta_y[i], names[i], \
                    text_size = -1, color = colors[i], rotation = rotation[i])
    #g.plot(graph.data.values(x=precisions,y=np.abs(data[ld_law]['500']['bias_p']/data[ld_law]['500']['precision_p']), title = None),\
    #                         styles = [graph.style.line([colors[i],\
    #                                   style.linestyle.dashed,\
    #                                   style.linewidth.thick])])
