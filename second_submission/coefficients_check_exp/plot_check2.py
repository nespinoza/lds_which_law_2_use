...
cbar = c.insert(graph.graphxy(height=7,width=0.5, xpos = g.xpos + 7.5, \
                                                ypos = g.ypos,\
       ...
       y2 = graph.axis.linear(min=np.min(value_range)-0.5,max=np.max(value_range)+0.5,\
                              manualticks = mymanualticks_y,title = r'$T_\textnormal{eff}$',parter=None)))

# "Paint" the plot:

for i in range(len(paint_tick_range)):
   the_color = get_color(paint_tick_range,i,half_val,m,n)
   draw_rectangular_band(cbar, -2., paint_value_range[i] - 0.5, 1., 1.0, the_color)
